...
-- ═════════════════════════════════════════════════════════════════════
-- 13. FINANCEIRO — DESPESAS RECORRENTES + DRE PROJETADO/REAL (de 015)
-- ═════════════════════════════════════════════════════════════════════
...
-- ═════════════════════════════════════════════════════════════════════
-- 14. NEGOCIOS COM EQUIPE_ID + RLS DE EQUIPES (de 016)
-- ═════════════════════════════════════════════════════════════════════
...
-- ═════════════════════════════════════════════════════════════════════
-- 15. AGENTES DE IA + WHATSAPP COM AGENTE ATIVO (de 017)
-- ═════════════════════════════════════════════════════════════════════
...
create trigger ai_agentes_updated_at before update on public.ai_agentes
  for each row execute procedure update_updated_at();

insert into public.ai_agentes (nome, descricao, modelo, system_prompt)
select 'Atendente Padrão',
       'Responde clientes no WhatsApp de forma cordial. Edite antes de ativar.',
       'claude-sonnet-4-6',
       'Você é uma atendente de uma corretora de seguros chamada CM.seg. Responda em português, cordial e direto. Não tome ações sensíveis sem confirmar com o cliente.'
where not exists (select 1 from public.ai_agentes);

-- ═════════════════════════════════════════════════════════════════════
-- 16. META CONVERSIONS API — eventos de CRM (de 018)
-- ═════════════════════════════════════════════════════════════════════

alter table public.meta_config add column if not exists dataset_id text;
...
-- ═════════════════════════════════════════════════════════════════════
-- 18. RD CRM PARITY: tags + origens + negocio_produtos + notas (de 020)
-- ═════════════════════════════════════════════════════════════════════

create table if not exists public.tags (
  id uuid primary key default uuid_generate_v4(),
  nome text not null, cor text default '#c9a84c',
  rd_id text, criado_em timestamptz default now()
);
...
create table if not exists public.origens (
  id uuid primary key default uuid_generate_v4(),
  nome text not null, rd_id text, ativo boolean default true,
  criado_em timestamptz default now()
);
...
alter table public.negocios add column if not exists origem_id uuid references public.origens(id) on delete set null;
...
create table if not exists public.negocio_notas (
  ...
  negocio_id uuid not null references public.negocios(id) on delete cascade,
  user_id uuid references public.users(id) on delete set null,
  conteudo text not null, criado_em timestamptz default now()
);
...
create table if not exists public.automacoes (
  ...
  trigger text not null check (trigger in ('negocio_criado','etapa_alterada','status_ganho','status_perdido')),
  funil_id uuid references public.funis(id) on delete cascade,
  etapa_filtro text,
...
create table if not exists public.automacoes_logs (
  ...
  negocio_id uuid references public.negocios(id) on delete set null,
  trigger text not null, sucesso boolean default true, erro text,
...
-- ═════════════════════════════════════════════════════════════════════
-- 23. EMAIL TEMPLATES (multiplos templates pra assinatura, de 025)
-- ═════════════════════════════════════════════════════════════════════
...
create table if not exists public.email_templates (
  ...
  assunto text, mensagem text not null,
  is_default boolean default false, ativo boolean default true,
  criado_por uuid references public.users(id),
  criado_em timestamptz default now(), atualizado_em timestamptz default now()
);
...
create trigger email_templates_atualizado before update on public.email_templates
  for each row execute procedure public.email_templates_set_atualizado();

do $$
declare v_legacy jsonb; v_assunto text; v_mensagem text;
begin
...
CM.seg — Corretora de Seguros');
    ...
    insert into public.email_templates (nome, categoria, assunto, mensagem, is_default)
    values
      ('Renovação de Apólice', 'assinatura', 'Renovação da sua apólice — assine para garantir',
'Olá {{cliente}},
...
CM.seg', false),
      ('Proposta Nova', 'assinatura', 'Sua proposta — {{documento}}',
'Olá {{cliente}},
...
-- ═════════════════════════════════════════════════════════════════════
-- 24. PORTO EXTRAS (campos da apolice + dados_porto JSONB, de 026)
-- ═════════════════════════════════════════════════════════════════════
