...
export default function ImportarRenovacoesPage() {
  ...
  async function importar() {
    ...
    if (!confirm(`Confirmar importação de ${linhas.length} linha(s)? Cada linha vira um card no funil RENOVAÇÕES atribuído à equipe RENOVAÇÕES.`)) return
    setImp(true)
    setRes(null)
    try {
      const { data: { session } } = await supabase.auth.getSession()
      const resp = await fetch('/api/importar/renovacoes', {
        method: 'POST',
  ...
  return (
    <div style={{padding:'24px 32px',maxWidth:1280,margin:'0 auto'}}>
      ...
      <h1 style={{fontFamily:'DM Serif Display,serif',fontSize:24,color:'var(--text)',marginBottom:8}}>💰 Importar Renovações</h1>
      <p style={{fontSize:13,color:'var(--text-muted)',marginBottom:20}}>
        Upload de planilha <strong>.xlsx</strong> com as colunas <code>NOME CLIENTE · APOLICE · VENCIMENTO · SEGURADORA</code>.
        Cada linha vira um card no funil <strong>RENOVAÇÕES</strong>, atribuído à equipe <strong>RENOVAÇÕES</strong>,
